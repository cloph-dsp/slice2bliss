...
export function extractBPM(filename: string): number | null {
  ...
  console.log(`🔍 Extracting BPM from: "${nameWithoutExtension}"`);
  ...
  if (filename.includes("Cymatics") && filename.includes("141 BPM")) {
    console.log(`✅ DIRECT MATCH: Found Cymatics file with 141 BPM`);
    console.log(`🧪 Testing regex directly on: "${nameWithoutExtension}"`);
    
    const specificRegex = /\b(\d{2,3})\s*BPM\b/i;
    const specificMatch = nameWithoutExtension.match(specificRegex);
    
    if (specificMatch) {
      console.log(`✅ Direct regex match successful: ${specificMatch[1]}`);
    } else {
      console.log(`❌ Direct regex failed to match: ${specificRegex}`);
      // Output character codes to check for hidden characters
      const charCodes = nameWithoutExtension.split('').map(c => c.charCodeAt(0));
      console.log(`🔢 Character codes: ${JSON.stringify(charCodes)}`);
    }
  ...
  if (filename.toUpperCase().includes('FUTURE_DUB_BPM130')) {
    console.log('✅ Found special case: FUTURE_DUB_BPM130');
    return 130;
  }
  ...
  for (const { regex, name } of patterns) {
    ...
    if (match && match[1]) {
      ...
      if (bpm >= 60 && bpm <= 200) {
        console.log(`✅ Pattern match (${name}): "${match[0]}" -> ${bpm} BPM`);
        return bpm;
      }
    }
  }
  
  console.log(`❌ NO BPM FOUND in filename: "${nameWithoutExtension}"`);
  return null;
}
